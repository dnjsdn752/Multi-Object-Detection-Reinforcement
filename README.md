...
정량 실험(테스트 평균 IoU) 관점에서 확인된 주요 결론은 다음과 같습니다.

- **타겟 할당 전략의 영향이 큼**
    
    distance 기반 타겟 지정 대비, **IoU 기반 타겟 지정이 두 에이전트 모두에서 평균 IoU를 유의미하게 개선**했습니다.
    
    → 멀티 에이전트 환경에서 “가까운 객체를 쫓기”보다 “현재 가장 잘 보고 있는 객체를 타겟으로 고정/갱신”하는 기준이 더 안정적으로 작동하는 경향이 나타났습니다.
    
- **‘서로 다른 객체를 보게 하려는’ 추가 reward는 개선이 제한적**
    
    서로 다른 GT를 유도하는 reward는 의도는 명확했지만, 평균 IoU가 오히려 감소하는 결과가 관찰되었습니다.
    
    → “중복 억제(분산)”가 곧바로 “정확한 탐지(수렴)”로 연결되지 않으며, 보상 설계는 분산과 수렴 목표를 동시에 만족해야 함을 시사합니다.
    
- **가장 확실한 개선은 DQN 2015 안정화(Target Network Update)**
    
    `iou_detach_5step`에서 **최고 평균 IoU**가 기록되었고, 학습의 안정성을 확보하는 데 기여했습니다.
    
    다만 mean_step이 증가해, “더 오래 탐색하지만 더 좋은 수렴”이라는 **정확도–효율 trade-off**가 존재했습니다.
    
- **협력형(Sigmoid Gating)은 평균 개선이 아닌 ‘비대칭 학습’이 관찰됨**
    
    협력 구조에서 agent2는 상승(≈0.59~0.60)했으나 agent1이 크게 하락(≈0.37)하며, **에이전트 간 성능이 불균형하게 형성**되는 현상이 나타났습니다.
    
    → 협력 메커니즘은 ‘정보 공유’ 자체보다 **역할 분화/균형을 유도하는 설계**가 동반되지 않으면 전체 성능 향상으로 이어지기 어려울 수 있습니다.
    

종합하면, 본 실험은 일부 결과가 기대만큼 향상되지는 않았으나, **타겟 할당 → 보상 설계 → 안정화 → 협력 구조**로 이어지는 확장 과정에서 “실제로 성능에 가장 크게 영향을 준 요인(타겟 할당, DQN 안정화)”과 “개선이 제한적이었던 요인(추가 reward, gating 협력의 불균형)”을 **정량 결과로 분리해 확인**했다는 점에서 의미가 있습니다.
